...
void SGX_UBRIDGE(SGX_NOCONVENTION, ocall_read_config_data, (const char* str));
void SGX_UBRIDGE(SGX_NOCONVENTION, ocall_read_log_messages, (const char* str));
void SGX_UBRIDGE(SGX_NOCONVENTION, ocall_listen_log_messages, ());
uint32_t SGX_UBRIDGE(SGX_NOCONVENTION, ocall_write_region_data, (uint8_t* blob, uint32_t bloblen));
uint32_t SGX_UBRIDGE(SGX_NOCONVENTION, ocall_read_region_data, (uint8_t* blob, uint32_t bloblen_in, uint32_t* bloblen_out));
uint32_t SGX_UBRIDGE(SGX_NOCONVENTION, ocall_write_sealed_data, (uint8_t* blob, uint32_t bloblen, const char* str));
uint32_t SGX_UBRIDGE(SGX_NOCONVENTION, ocall_read_sealed_data, (char* str));
